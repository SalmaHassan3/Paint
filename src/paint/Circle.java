...
Ellipse2D circle;
private Color color,fillColor=null;
boolean selected;
 AffineTransform at;
Circle(){
    ...
    public void ellipticalShape2D(float xstart, float xend, float ystart, float yend) {
        ...
        this.ystart = ystart;
        this.yend = yend;
        circle =new Ellipse2D.Float((float)xstart, (float)ystart,(float)xend-(float)xstart,(float)xend-(float)xstart);
    }
 @Override
...
@Override
    ...
    at =AffineTransform.getRotateInstance(Math.toRadians(angle) ,circle.getCenterX(),circle.getCenterY());
    paintJFrame.g2.setTransform(at);
    if(fillColor!=null){
        paintJFrame.g2.setPaint(fillColor);
        paintJFrame.g2.fill(circle);
    }
          paintJFrame.g2.draw(circle);
           if(selected==true){
    Stroke dotted = new BasicStroke(1, BasicStroke.CAP_BUTT, BasicStroke.JOIN_BEVEL, 0, new float[] {1,2}, 0);
            Rectangle2D selection=new Rectangle2D.Float((float)circle.getBounds2D().getMinX()-20, (float)circle.getBounds2D().getMinY()-20,(float)circle.getBounds2D().getWidth()+40 ,(float)circle.getBounds2D().getHeight()+40);
